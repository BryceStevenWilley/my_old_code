...
public class Song 
{
     private ArrayList song;   // 1st melody notes
     private ArrayList song2;  // 2nd melody notes
    ...
    public Song(int y)
     {
         ...
         song2= new ArrayList(y);
         intervals= new ArrayList(y*2);
         intervals2= new ArrayList(y*2);
    ...
    public Song(Song s)
    {
        ...
        song2= (ArrayList)s.song2.clone();
        notes= s.notes;
        beat= s.beat;
        intervals= (ArrayList)s.intervals.clone();
        intervals= (ArrayList)s.intervals2.clone();
    ...
    public void writeNote(int place, char n, char sf, int oct, double ori)
    {
     ...
     public void writeNote2(int place, char n, int oct)
    {
        System.out.println("Adding a new Note to the song.");
        Note insert= new Note(n, oct);
        song2.add(place , insert);
        notes++;
             
    }
    public void writeNote2(int place, char n, char sf, int oct)
    {
        System.out.println("Adding a new Note to the song.");
        Note insert =new Note( n, sf, oct);
        song2.add(place , insert);
    ...
    public void writeNote2(int place, char n, char sf, int oct, double ori)
    {
        System.out.println("Adding a new Note to the song.");
        Note insert= new Note(n, sf, oct, ori);
        song2.add(place, insert);
    ...
    public void editNote(int place, double ori)
    {
     ...
     public void editNote2(int place, double ori)
    {
        System.out.println("Editing an established note/ adding orgin.");
        String name= getNoteName(place);
        char n= name.charAt(0);
    ...
    public void deleteNote(int place)
    {
     ...
     public void deleteNote2(int place)
    {
        System.out.println("Deleting Note at " + place+ ".");
        song2.remove(place);
        notes--;
     ...
     public void findIntervalsFirst()
    {
        ...
        for (int i =1; i < song.size(); i++)
        {
            ...
            if(lyric.getOrgin()>= lyric2.getOrgin())
            {intervals.add(new Integer(Math.abs(lyric2.getDistance()-lyric.getDistance())));}
            else if(lyric.getOrgin()< lyric2.getOrgin())
            {  j=1;
                while (j< i)
              {if(lyric.getOrgin() > ((Note)song.get(i-j)).getOrgin())
                 {lyric3=(Note)song.get(i-j);
                  intervals.add(new Integer(Math.abs(lyric3.getDistance()-lyric.getDistance())));
                  break;
                  }           
               j++;
              }          
 ...
 //   int interval;
   ...
   public Note(char n, int oct)
   {
     ...
     disFromC= oct * 12;
     switch (n){
     case 'a': disFromC += 9; break;
     case 'b': disFromC +=11; break;
     case 'c': disFromC +=0; break;
     case 'd': disFromC +=2;  break;
     case 'e': disFromC +=4;  break;
     case 'f': disFromC +=5;  break;
     case 'g': disFromC +=7; break;
     }
